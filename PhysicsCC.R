...
#val$Outcome<-ifelse(val$CF..ansbin.==1,"CORRECT","INCORRECT")
...
testKCmodel<-function (iter,posKC,KCthreshm,RSVDcomp,val){
 ...
 # View(val$AC)
  ...
  #View(val)
  modelob<-LKT(data=(val),components=c("Anon.Student.Id","KC..Default.","KC..Default.","KC..Default.","AC","AC"),
  ...
  modelob2<-LKT(data=(val),components=c("Anon.Student.Id","KC..Default.","KC..Default.","KC..Default.","AC","AC"),
                features=c("intercept","intercept","logsuc$","logfail$","logsuc$","logfail$"),
                fixedpars=c(.9,.7),interc=TRUE,verbose=FALSE)

  cat(paste(posKC,KCthreshm,RSVDcomp,modelob$r2,
            modelob2$r2,mean(modelob$subjectrmse$x),
            mean(modelob2$subjectrmse$x),(modelob$r2-modelob2$r2)/((modelob$r2+modelob2$r2)/2),"\n",sep=","))
  x<<-rbind(x,c(posKC,RSVDcomp,(modelob$r2-modelob2$r2)/((modelob$r2+modelob2$r2)/2)))
  y1<<-modelob
  y2<<-modelob2
...
temp<-grid_search(testKCmodel,params=list(KCthreshm=c(.1),RSVDcomp=c(2,3,4,5,6,7,8,9,10),posKC=c(2,3,4,5,6,7,8,10,12,14,16)),val=val)
...
bar(dv = "R-squared_Gain",
 ...
 LKT(data=rlvl(val),components=c("KC..Default.","KC..Default."),
     features=c("linesuc$","linefail$"),
     fixedpars=c(.9,.7),interc=TRUE,verbose=FALSE)$coefs
